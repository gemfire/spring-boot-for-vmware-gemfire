...
get-spring-boot-data-gemfire-version() {
...
get-spring-boot-data-gemfire-version-qualifier-slug() {
  local CONCOURSE_VERSION=$1
...
get-spring-boot-data-gemfire-build-id() {
  local CONCOURSE_VERSION=$1
...
get-spring-boot-data-gemfire-build-id-padded() {
  local CONCOURSE_VERSION=$1
  local BUILD_ID=$(get-spring-boot-data-gemfire-build-id ${CONCOURSE_VERSION})
...
get-full-version() {
  ...
  local CONCOURSE_VERSION=$1
  local SPRING_DATA_GEMFIRE_PRODUCT_VERSION=$(get-spring-boot-data-gemfire-version ${CONCOURSE_VERSION})
  local QUALIFIER_SLUG=$(get-spring-boot-data-gemfire-version-qualifier-slug ${CONCOURSE_VERSION})
  local PADDED_BUILD_ID=$(get-spring-boot-data-gemfire-build-id-padded ${CONCOURSE_VERSION})
