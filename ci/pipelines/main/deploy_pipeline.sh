...
SDG_FORK=$(yq -r .repositories.springBootDataGemfire.fork \
  ${SCRIPT_DIR}/../shared/jinja.variables.yml)

SDG_BRANCH=$(yq -r .repositories.springBootDataGemfire.branch \
  ${SCRIPT_DIR}/../shared/jinja.variables.yml)
...
pushd ${SCRIPT_DIR} 2>&1 > /dev/null
  ...
  fly -t ${TARGET} set-pipeline \
    ...
    --var spring-boot-data-gemfire-build-branch=${SANITIZED_SDG_BRANCH} \
