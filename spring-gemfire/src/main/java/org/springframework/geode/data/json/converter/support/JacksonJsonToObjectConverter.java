...
/**
 * {@link JsonToObjectConverter} implementation using Jackson to convert {@link String JSON} to an {@link Object}
 * (POJO).
 *
 * @author John Blum
 * @see JsonNode
...
public class JacksonJsonToObjectConverter implements JsonToObjectConverter {
	...
	private @NonNull ObjectMapper newObjectMapper() {
		...
		return new ObjectMapper().configure(DeserializationFeature.FAIL_ON_IGNORED_PROPERTIES, false)
				...
				.configure(MapperFeature.ACCEPT_CASE_INSENSITIVE_ENUMS, true).findAndRegisterModules();
	}

	/**
	 * Returns a reference to the configured Jackson {@link ObjectMapper} used by this {@link Converter} to convert
	 * {@link String JSON} into an {@link Object} (POJO).
	 *
		...
		if (StringUtils.hasText(json)) {
			...
			try {
				...
				if (isPojo(jsonNode)) {
					...
					Assert.state(jsonNode.has(AT_TYPE_FIELD_NAME), () -> String
							.format("The JSON object [%1$s] must have an '%2$s' metadata field", json, AT_TYPE_FIELD_NAME));

					objectTypeName = jsonNode.get(AT_TYPE_FIELD_NAME).asText();

					Class<?> objectType = ClassUtils.forName(objectTypeName, Thread.currentThread().getContextClassLoader());

					return objectMapper.readValue(json, objectType);
				}
			} catch (ClassNotFoundException cause) {
				throw new MappingException(
						String.format("Failed to map JSON [%1$s] to an Object of type [%2$s]", json, objectTypeName), cause);
